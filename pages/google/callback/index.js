...
export default function GoogleCallback() {
    ...
    useEffect(() => {
        ...
        const doCallback = async () => {
            ...
            const redirectUri = resolveSocialRedirectUri("google");
            ...
            try {
                ...
                if (!token || !member) {
                    console.error('[GoogleCallback] 로그인 응답 문제', data);
                    alert('구글 로그인 실패: 서버 응답이 올바르지 않습니다. 콘솔을 확인하세요.');
                    router.push('/login');
                    return;
                }
                ...
                console.error('[GoogleCallback] 소셜 로그인 에러', err);
                alert('구글 로그인 처리 중 오류가 발생했습니다: ' + (err.message || '알 수 없음'));
                router.push('/login');
            }
        };
